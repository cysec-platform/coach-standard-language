...
public abstract class Command {
    ...
    public Atom checkAtomType(
            Atom atom, List<AtomType> type, boolean evaluate, CoachContext context, String parameterName)
            throws ExecutorException {
        if (atom == null) {
            throw new RuntimeException(
                    "SEVERE ERROR: ATOM was passed as null when checking atom type... please check calling function");
        }
    ...
    public void checkNumParams(List<Atom> aList, int lowNum, int highNum) throws ExecutorException {
        if (aList.size() > highNum || aList.size() < lowNum) {
            if (highNum == lowNum) {
                throw new ExecutorException("Invalid number of arguments. Expected " + lowNum
                        + " parameters but got " + aList.size() + " parameters in command "
                        + getCommandName() + ".");
            } else {
                throw new ExecutorException("Invalid number of arguments. Expected between "
                        + lowNum + " and " + highNum
                        + " parameters but got "
