...
public class TestParserBasicFunctionality {
    ...
    public void setup() {
        ...
        coachContext = new CoachContext(
                CySeCExecutorContextFactory.getExecutorContext("test"), cal, question, null, null, null);
    ...
    public void testBasicAddCommand() throws Exception {
        ...
        assertTrue(
                "Execution result is not as expected (" + a.execute(coachContext) + ")",
    ...
    public void testIfEvaluation() {
        try {
            ...
            a = new ParserLine(s).getAtom().execute(coachContext);
            assertTrue(
                    "content missmatch for string (condition not evaluated properly? a=" + a + ")",
                    "10".equals(a.getId()));
            context.reset();
            s = "if( and(FALSE,FALSE), addScore(\"test\",5), addScore(\"test\",10))";
            a = new ParserLine(s).getAtom().execute(coachContext);
            assertTrue(
                    "content missmatch for string (eval test 1; score="
                            + context.getScore("test").getValue() + ")",
                    context.getScore("test").getValue() == 10);
            context.reset();
            s = "if( and(TRUE,TRUE), addScore(\"test\",5), addScore(\"test\",10))";
            a = new ParserLine(s).getAtom().execute(coachContext);
            assertTrue(
                    "content missmatch for string (eval test 2; score="
                            + context.getScore("test").getValue() + ")",
                    context.getScore("test").getValue() == 5);
        } catch (Exception pe) {
            pe.printStackTrace();
            fail("got unexpected exception");
        }
    ...
    public void testCySeCEvalOrder() {
        ...
        try {
            ...
            assertTrue(
                    "content missmatch for string (eval test 1; score="
                            + context.getScore("test").getValue() + ")",
                    context.getScore("test").getValue() == 2);
            context.executeQuestion(
                    new ParserLine(sb.toString().replaceAll("or\\(FALSE,FALSE\\)", "TRUE")).getCySeCListing(),
                    coachContext);
            assertTrue(
                    "content missmatch for string (eval test 2; score="
                            + context.getScore("test").getValue() + ")",
                    context.getScore("test").getValue() == 8);
            context.reset();
            context.executeQuestion(
                    new ParserLine(sb.toString().replaceAll("and\\(FALSE,FALSE\\)", "TRUE")).getCySeCListing(),
                    coachContext);
            assertTrue(
                    "content missmatch for string (eval test 3; score="
                            + context.getScore("test").getValue() + ")",
                    context.getScore("test").getValue() == 24);
            context.executeQuestion(
                    new ParserLine(sb.toString()
                                    .replaceAll("or\\(FALSE,FALSE\\)", "TRUE")
                                    .replaceAll("and\\(FALSE,FALSE\\)", "TRUE"))
                            .getCySeCListing(),
                    coachContext);
            assertTrue(
                    "content missmatch for string (eval test 4; score="
                            + context.getScore("test").getValue() + ")",
                    context.getScore("test").getValue() == 8);
        } catch (Exception pe) {
            pe.printStackTrace();
    ...
    public void testConcatenation() {
        try {
            ...
            assertTrue(
                    "content missmatch for CONCAT(STRING) [" + a.getType() + "=" + a.getId() + "]",
                    "a".equals(a.getId()));
    ...
    public void testVariables() {
        try {
            assertTrue(
                    "illegal var content (1) [" + context.getVariable("A", null) + "]",
                    context.getVariable("A", null) == NULL_ATOM);
            Atom a = new ParserLine("set(\"A\",\"A\",\"A\")").getAtom();
            a.execute(coachContext);
            assertTrue(
                    "illegal var content (2) [" + context.getVariable("A", null) + "]",
                    "A".equals(context.getVariable("A", null).getId()));
            a = new ParserLine("set(\"A\",\"B\",\"B\")").getAtom();
            a.execute(coachContext);
            assertTrue(
                    "illegal var content (3) [" + context.getVariable("A", null) + "]",
                    "B".equals(context.getVariable("A", null).getId()));
            assertTrue(
                    "illegal var content (4) [" + context.getVariable("A", "A") + "]",
                    "A".equals(context.getVariable("A", "A").getId()));
            assertTrue(
                    "illegal var content (5) [" + context.getVariable("A", "B") + "]",
                    "B".equals(context.getVariable("A", "B").getId()));
            a = new ParserLine("set(\"A\",NULL,\"C\")").getAtom();
            a.execute(coachContext);
            assertTrue(
                    "illegal var content (6) [" + context.getVariable("A", null) + "]",
                    "C".equals(context.getVariable("A", null).getId()));
            assertTrue(
                    "illegal var content (7) [" + context.getVariable("A", "A") + "]",
                    "A".equals(context.getVariable("A", "A").getId()));
            assertTrue(
                    "illegal var content (8) [" + context.getVariable("A", "B") + "]",
                    "B".equals(context.getVariable("A", "B").getId()));
            a = new ParserLine("set(\"A\",\"A\",\"D\")").getAtom();
            a.execute(coachContext);
            assertTrue(
                    "illegal var content (9) [" + context.getVariable("A", null) + "]",
                    "D".equals(context.getVariable("A", null).getId()));
            assertTrue(
                    "illegal var content (10) [" + context.getVariable("A", "A") + "]",
                    "D".equals(context.getVariable("A", "A").getId()));
            assertTrue(
                    "illegal var content (11) [" + context.getVariable("A", "B") + "]",
                    "B".equals(context.getVariable("A", "B").getId()));
        } catch (ParserException pe) {
            pe.printStackTrace();
            fail("got unexpected exception");
